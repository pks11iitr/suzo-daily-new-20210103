    ...
    public function initiateOrder(Request $request){
        ...
        foreach($cart as $item) {
            ...
            if($item->type=='subscription'){
                ...
                $daywise_delivery_total[$item->start_date]=$daywise_delivery_total[$item->start_date]+$item->product->price*$item->total_quantity;
...
//            Wallet::updatewallet($order->user_id, 'Refund for item cancellation from order id: '.$order->refid, 'Credit',$point_return, 'POINT', $order->id);
    ...
    private function cancelSubscription($user, $detail,$message){
        ...
        if($detail->product->subscription_cashback){
            ...
            return [
                'status'=>'failed',
                'message'=>'This subscription includes additional Frestr Cash benefits. Please raise a case in complaints section to cancel this product'
            ];
...
//        ];
    ...
    public function raiseReturn(Request $request, $id){
        ...
        $detail=OrderDetail::whereHas('order', function($order) use($user){
                $order->where('user_id', $user->id);
            })->whereIn('order_details.status', ['completed', 'partially-completed'])
            ->where('type', 'once')
            ->findOrFail($id);

        if(($detail->total_quantity-$detail->cancel_returned) < $request->quantity)
            return [
                'status'=>'failed',
                'message'=>'Return quantity cannot exceed purchased quantity'
            ];

        // can be returned within two days
        ...
        if(isset($detail->deliveries[0]) && in_array($detail->deliveries[0]->status, ['approved','rejected'] ))
        return [
            'status'=>'failed',
            'message'=>'This product cannot be returned now'
        ];
